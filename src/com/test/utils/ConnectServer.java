...
public class ConnectServer {
	...
	public static void getResualt(final Context context,
			HashMap<String, String> paramter, final NetworkAction request,
			String url) {
		...
		MyApplication.client.postWithURL(url, paramter,
				new Listener<JSONObject>() {
						...
						try {
							...
							if (code == 1 || code == -1) {
								if (context instanceof MenuActivity)
									((MenuActivity) context).showResualt(response,
											request);
								else
									((NormalActivity) context).showResualt(
											response, request);
							} else {
					...
					public void onErrorResponse(VolleyError error) {
						Toast.makeText(context, "访问服务器出错!"+error.getMessage(), 2000).show();
						Log.i(MyApplication.TAG,
